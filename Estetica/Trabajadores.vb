...
Public Class Trabajadores
    ...
    Dim TableName As String = "TRABAJADOR"
    Dim IdTrabajador, NombreTrabajador, SueldoTrabajador As String
    ...
    Private Sub Trabajadores_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim SQL As String = "select*  from " & TableName & " order by NombreTrabajador"
    ...
    Private Sub btnRegistrar_MouseClick(sender As Object, e As MouseEventArgs) Handles btnRegistrar.MouseClick
        ...
        If txtId.Text = "" And txtNombre.Text = "" And txtSalario.Text = "" Then
        ...
        Try
            ExecuteNonQuery("insert into " & TableName & " (IdTrabajador, NombreTrabajador, SueldoTrabajador) values ('" & txtId.Text & "','" & txtNombre.Text _
                            & "','" & txtSalario.Text & "')", SQLiteCon)
    ...
    Private Sub btnRefrescar_MouseClick(sender As Object, e As EventArgs) Handles btnRefrescar.Click
        ...
        Try
            LoadDB("select*from " & TableName & " order by NombreTrabajador", TableDB, SQLiteCon)
            DataGridViewTable.DataSource = Nothing
            DataGridViewTable.DataSource = TableDB
            DataGridViewTable.ClearSelection()
        Catch ex As Exception
...
#Disable Warning BC42017 ' Resolución enlazada en tiempo de ejecución
                    ExecuteNonQuery(query:=$"delete from {TableName} where IdTrabajador='{row.DataBoundItem(0).ToString}'", SQLiteCon)
#Enable Warning BC42017 ' Resolución enlazada en tiempo de ejecución
    ...
    Private Sub btnModificar_MouseClick(sender As Object, e As EventArgs) Handles btnModificar.Click
        ...
        Try
            ExecuteNonQuery("update " & TableName & " set NombreTrabajador='" & txtNombre.Text & "',SueldoTrabajador='" & txtSalario.Text _
                            & "' WHERE IdTrabajador = '" & txtId.Text & "'", SQLiteCon)
    ...
    Private Sub btnCrearId_MouseClick(sender As Object, e As MouseEventArgs) Handles btnCrearId.MouseClick
        ...
        Try
            LoadDB("select IdTrabajador from " & TableName & " where IdTrabajador ='" & txtId.Text & "'", TableDB, SQLiteCon)
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub buscar_TextChanged(sender As Object, e As EventArgs) Handles buscar.TextChanged
        ...
        If buscar.Text = Nothing Then
            SearchCMD = "select*from " & TableName & " order by NombreTrabajador"
        Else
            SearchCMD = "select*from " & TableName & " where NombreTrabajador like'" & buscar.Text & "%'"
            SearchCMD = "select*from " & TableName & " where NombreTrabajador like'" & buscar.Text & "%'"
        End If
    ...
    Private Sub ToolStripMenuItemEditar_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItemEditar.Click
        ...
        If AllCellsSelected(DataGridViewTable) = False Then
            txtId.Text = IdTrabajador
            txtNombre.Text = NombreTrabajador
            txtSalario.Text = SueldoTrabajador
            txtId.Enabled = False
            ...
            If e.Button = MouseButtons.Right Then
                ...
                With DataGridViewTable
                    If e.RowIndex >= 0 Then
                        ...
                        IdTrabajador = .Rows(i).Cells("IdTrabajador").Value.ToString
                        NombreTrabajador = .Rows(i).Cells("NombreTrabajador").Value.ToString
                        SueldoTrabajador = .Rows(i).Cells("SueldoTrabajador").Value.ToString
                    End If
                End With
            End If
        End If
